...
function getNodesStatus(obj) {
    ...
    // Check if the object has children (not a leaf node)
...
function handlePartSelection(event) {
    ...
    fetch('http://127.0.0.1:5000/parts', {
        ...
        // Access the curr_part value from the response
...
function drawTree(treeData) {
    ...
    function collapse(d) {
        ...
        if (openedNodesState[currPart][section] === false) {
            // Collapse this node
            if (d.children) {
                d._children = d.children;  // Save the children to _children property
                d.children = null;  // Remove the children (collapse) 
